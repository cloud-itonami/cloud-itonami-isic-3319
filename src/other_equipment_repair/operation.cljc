(ns other-equipment-repair.operation
  "OperationActor -- one repair-of-other-equipment-shop coordination
  ...
  decision to a human operator (the licensed repair technician). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:flag-safety-concern` ALWAYS reaches this node -- see
  `other-equipment-repair.phase`/`other-equipment-repair.governor` ns
  docstrings. `:log-repair-record`, `:schedule-repair-operation` (when
  governor-clean) and `:order-supplies` (below the cost threshold) MAY
  auto-commit at phase 3 when the governor is clean -- see `other-
  equipment-repair.phase` ns docstring.

  Every committed record carries `:effect :propose` (see `other-
  ...
  `other-equipment-repair.notify`) to the equipment/work-order's
  `:safety-contacts` roster -- always AFTER a human has approved it,
  since `:flag-safety-concern` is never auto-eligible at any phase."
...
(defn build
  ...
  `other-equipment-repair.store/Store`).
  opts:
    :advisor      -- an `other-equipment-repair.advisor/Advisor` (default: mock-advisor)
    :notifier     -- an `other-equipment-repair.notify/Notifier` (default: mock-notifier)
        ...
        (fn [{:keys [request context proposal record]}]
          ...
          (let [f (commit-fact request context proposal)
                notify-result (when (= :flag-safety-concern (:op record))
                                (let [eq (store/equipment store (:subject request))]
                                  (notify/dispatch-safety-concern-notice!
                                   notifier (:safety-contacts eq) (:value proposal))))]
